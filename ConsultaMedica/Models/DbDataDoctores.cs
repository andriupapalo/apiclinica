...
namespace ConsultaMedica.Models
{
    ...
    public class DbDataDoctores : DbContext
    {
        ...
        public Doctor Update(Doctor doct, int id)
        {
            var paci = Doctor.FirstOrDefault(x => x.Id == id);
            try
            {
                if (paci != null)
                {
                    paci.NumeroCredencial = doct.NumeroCredencial;
                    paci.NombreCompleto = doct.NombreCompleto;
                    paci.Especialidad = doct.Especialidad;
                    paci.TelefonoContacto = doct.TelefonoContacto;
                    paci.Email = doct.Email;
                    paci.HospitalDeTrabajo = doct.HospitalDeTrabajo;
                    Entry(paci).State = EntityState.Modified;
                    SaveChanges();
                }
            }
